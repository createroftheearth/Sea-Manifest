$(function () {
    initTransportEquipmentHouseCargos();
});

var TransportEquipmentHouseCargoTable;

function initTransportEquipmentHouseCargos() {
    TransportEquipmentHouseCargoTable = $('#tblTransportEquipmentHouseCargo').DataTable({
        "searching": true,
        "ordering": false,
        "processing": true,
        "serverSide": true,
        ...
        "filter": true,
        "ajax": {
            "url": "/TransportEquipmentHouseCargo/GetTransportEquipmentHouseCargo",
            "type": "POST",
        },
        "columns": [
            {
            ...
            },
            {
                "data": "iTransporterEquipmentId", "mRender": function (data) {
                    return "<button type=\"button\" class=\"btn btn-warning btn-xs not-hide\" onClick=\"AddUpdateTransportEquipmentHouseCargo(" + data + ")\"><i class=\"fa fa-edit\"></i> Edit</button> ";
                }
            },
        ]
    });
...
function AddUpdateTransportEquipmentHouseCargo(iTransportEquipmentHouseCargoId) {
    $('#addUpdateModallgContainer').load('/TransportEquipmentHouseCargo/AddUpdateTransportEquipmentHouseCargo?iTransporterEquipmentId=' + iTransportEquipmentHouseCargoId, function () {
        initAddUpdateTransportEquipmentHouseCargo();
    });
}

function initAddUpdateTransportEquipmentHouseCargo() {
    $.validator.unobtrusive.parse('#frmTransportEquipmentHouseCargo');
    $('#frmTransportEquipmentHouseCargo select').selectpicker();
...
$(document).on('submit', '#frmTransportEquipmentHouseCargo', function (e) {
    e.preventDefault();
        ...
        $.ajax({
            ...
            success: function (response) {
                if (response.Status) {
                    ...
                    TransportEquipmentHouseCargoTable.ajax.reload();
                    $('.modal').modal('hide');
                } else {
